...
const TreeSelectJPK: FC<TreeSelectJPKProps> = (props) => {
  ...
  const onCheck = (checked: React.Key[], e: any) => {
    console.log(checked, checked.concat(e.halfCheckedKeys?.map((item: string) => `${item}-half`)));
    setSelectedValue(checked.concat(e.halfCheckedKeys?.map((item: string) => `${item}-half`)));

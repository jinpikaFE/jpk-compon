...
const FormUploadImg: React.FC<TFormUploadImg> = (props) => {
  const { fileSize, formItemProps, uploadProps, required, extraUploadFn, uploadFileReq } = props;
  ...
  const handleUpload = async (info: any) => {
    ...
    if (file?.size / 1024 < (fileSize || 0)) {
      ...
      info.onError(
        `图片大小超出${
          (fileSize || 512) > 1024 ? Math.fround((fileSize || 512) / 1024) + 'M' : fileSize + 'k'
        }`,
      );
      setIisOutSize(true);
      message.error(
